    ...
    <x-container>
        ...
        <section class="mt-[35px] grid grid-cols-1 justify-between lg:grid-cols-2 xl:gap-0 gap-10">
            <div class="justify-self-center md:justify-self-start">
                ...
                {{-- <img src="/dummy/sell_1.png"
                    class="md:h-[500px] md:w-[500px] h-full w-full object-cover rounded-t-[10px]" alt="" /> --}}


                {{-- button wishlist --}}
                <div class="mt-1 flex justify-center gap-10 py-4 px-2 border-gray-3 border rounded-b-[10px]">
                    <form action="{{ route('auctions.wishlist', $auction->id) }}" method="POST"
                        class="flex items-center gap-2 w-[48%] justify-center">
                        @csrf
                        <button type="submit" class="flex items-center gap-2 justify-center">
                            Wishlist
                            @if($wishlisted)
